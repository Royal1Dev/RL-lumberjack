...
RegisterNetEvent('RL-lumberjack:sellItems', function()
    local source = source
    local price = 0
...
RegisterNetEvent('RL-lumberjack:BuyAxe', function()
    local source = source
...
QBCore.Functions.CreateCallback('RL-lumberjack:axe', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player ~= nil then
...
RegisterNetEvent('RL-lumberjack:setLumberStage', function(stage, state, k)
    Config.TreeLocations[k][stage] = state
    TriggerClientEvent('RL-lumberjack:getLumberStage', -1, stage, state, k)
end)

RegisterNetEvent('RL-lumberjack:setChoppedTimer', function()
    if not Chopped then
        Chopped = true
        CreateThread(function()
            ...
            for k, v in pairs(Config.TreeLocations) do
                ...
                TriggerClientEvent('RL-lumberjack:getLumberStage', -1, 'isChopped', false, k)
            end
            Chopped = false
        end)
    end
...
RegisterServerEvent('RL-lumberjack:recivelumber', function()
    local source = source
    local Player = QBCore.Functions.GetPlayer(tonumber(source))
...
QBCore.Functions.CreateCallback('RL-lumberjack:lumber', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player ~= nil then
...
RegisterServerEvent('RL-lumberjack:lumberprocessed', function()
    local source = source
    local Player = QBCore.Functions.GetPlayer(tonumber(source))
